...
def render(df_all, lang):
    ...
    with c1:
        ...
        st.dataframe(_make_style(disp.head(10), _t(lang, "score")), width="stretch", hide_index=True)
    with c2:
        st.markdown(f"**{_t(lang, 'bottom')}**")
        st.dataframe(_make_style(disp.tail(10), _t(lang, "score")), width="stretch", hide_index=True)
